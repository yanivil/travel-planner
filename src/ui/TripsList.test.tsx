...
describe('TripsList', () => {
  ...
  test('delete asks for confirmation and cascades days and stops', async () => {
    ...
    const user = userEvent.setup();
    await db.trips.add({ id: 'trip-1', name: 'Doomed', createdAt: '2026-08-01T00:00:00.000Z', maxDriveStretchMin: null, observance: 'none' });
    await db.days.add({ id: 'd1', tripId: 'trip-1', index: 0, title: 'Day 1', startMin: 480, zone: 'Asia/Jerusalem', curfewMin: null, lat: null, lng: null, locationName: null });
  ...
  test('declining the confirmation keeps the trip', async () => {
    ...
    const user = userEvent.setup();
    await db.trips.add({ id: 'trip-1', name: 'Safe', createdAt: '2026-08-01T00:00:00.000Z', maxDriveStretchMin: null, observance: 'none' });
